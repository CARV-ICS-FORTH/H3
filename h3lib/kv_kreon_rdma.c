...
typedef struct {
    char* ip;
    int port;
}KV_Kreon_RDMA_Handle;

void KV_Kreon_RDMA_Free(KV_Handle _handle) {
	KV_Kreon_RDMA_Handle* handle = (KV_Kreon_RDMA_Handle*) _handle;
	krc_close();
	free(handle->ip);
	free(handle);
    return;
...
KV_Handle KV_Kreon_RDMA_Init(const char* storageUri) {
    ...
    KV_Kreon_RDMA_Handle* handle = malloc(sizeof(KV_Kreon_RDMA_Handle));
    krc_ret_code status;
...
KV_Status KV_Kreon_RDMA_List(KV_Handle handle, KV_Key prefix, uint8_t nTrim, KV_Key buffer, uint32_t offset, uint32_t* nKeys){
...
KV_Status KV_Kreon_RDMA_Exists(KV_Handle _handle, KV_Key key) {
...
KV_Status KV_Kreon_RDMA_Read(KV_Handle handle, KV_Key key, off_t offset, KV_Value* value, size_t* size) {
	KV_Status status;
...
KV_Status KV_Kreon_RDMA_Update(KV_Handle handle, KV_Key key, KV_Value value, off_t offset, size_t size) {
    KV_Status status;
...
KV_Status KV_Kreon_RDMA_Write(KV_Handle handle, KV_Key key, KV_Value value, size_t size) {
...
KV_Status KV_Kreon_RDMA_Delete(KV_Handle handle, KV_Key key) {
...
KV_Status KV_Kreon_RDMA_Create(KV_Handle _handle, KV_Key key, KV_Value value, size_t size){
	KV_Status status;
	KV_Kreon_RDMA_Handle* handle = (KV_Kreon_RDMA_Handle *)_handle;

	if( (status = KV_Kreon_RDMA_Exists(handle, key)) == KV_KEY_NOT_EXIST){
		 status = KV_Kreon_RDMA_Write(handle, key, value, size);
	}

	return status;
}

KV_Status KV_Kreon_RDMA_Copy(KV_Handle _handle, KV_Key src_key, KV_Key dest_key) {
    size_t size = 0x00;
    KV_Value value = NULL;
	KV_Status status;
	KV_Kreon_RDMA_Handle* handle = (KV_Kreon_RDMA_Handle *)_handle;

	if((status = KV_Kreon_RDMA_Read(handle, src_key, 0, &value, &size)) == KV_SUCCESS){
		status = KV_Kreon_RDMA_Write(handle, dest_key, value, size);
	}

	return status;
}

KV_Status KV_Kreon_RDMA_Move(KV_Handle _handle, KV_Key src_key, KV_Key dest_key) {
    size_t size = 0x00;
    KV_Value value = NULL;
	KV_Status status;
	KV_Kreon_RDMA_Handle* handle = (KV_Kreon_RDMA_Handle *)_handle;
	...
	if( (status = KV_Kreon_RDMA_Read(handle, src_key, 0, &value, &size)) == KV_SUCCESS &&
		(status = KV_Kreon_RDMA_Write(handle, dest_key, value, size)) == KV_SUCCESS		){
		status = KV_Kreon_RDMA_Delete(handle, src_key);
	}

	return status;
}

KV_Status KV_Kreon_RDMA_Sync(KV_Handle handle) {
    return KV_SUCCESS;
}
...
KV_Operations operationsKreonRDMA = {
    ...
    .metadata_write = KV_Kreon_RDMA_Write,
    .metadata_create = KV_Kreon_RDMA_Create,
    .metadata_delete = KV_Kreon_RDMA_Delete,
    .metadata_move = KV_Kreon_RDMA_Move,
    .metadata_exists = KV_Kreon_RDMA_Exists,

    .list = KV_Kreon_RDMA_List,
    .exists = KV_Kreon_RDMA_Exists,
    .read = KV_Kreon_RDMA_Read,
    .create = KV_Kreon_RDMA_Create,
    .update = KV_Kreon_RDMA_Update,
    .write = KV_Kreon_RDMA_Write,
    .copy = KV_Kreon_RDMA_Copy,
    .move = KV_Kreon_RDMA_Move,
    .delete = KV_Kreon_RDMA_Delete,
    .sync = KV_Kreon_RDMA_Sync
};
