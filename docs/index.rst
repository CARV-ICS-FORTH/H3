...
H3 is implemented in the ``h3lib`` library, which provides a cloud-friendly API, similar to Amazon's S3. On the backend, H3 supports `RocksDB <https://rocksdb.org>`_ for single-node runs, `Redis <https://redis.io>`_ for over-the-network storage, Kreon for local or distributed storage deployments, as well as plain files for layering on top of any filesystem (and easy testing).
